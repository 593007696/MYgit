...
<?php
    ...
    if($r=="確認") {
        ...
        $member_name_db =$result['last_name'].$result['first_name'];
        ...
        foreach($ds as $d) {
            ...
            if(( $service_reserve_flag=="1")  /**&& ($start_day==$start_date)**/ ){//コワーキングが存在するか確認
                if (($member_id ==$result['member_id']) && ($member_name == $member_name_db)) {  //会員情報がDB内に存在しているか確認 
                    ...
                    echo '<font color="RED">会員情報が間違っています!</font><br />';
                    echo '<form>';
                    echo '<input type="button" onclick="history.back()" value="戻る">';
                    echo '</form>';
                }
        ...
        echo '<p>ご利用の店舗：'.$_POST['store_id'];
        echo '<p>ご利用の部屋：'.$_POST['room_id'];
        echo '<p>会員ID：<input type="text" name="member_id" required="required"></p>';
        echo '<p>会員名：<input type="text" name="member_name" required="required"></p>';
        echo '<p for="date">日付:';
        ...
        <input type="submit" name="r" value="確認" >
        <?php
