...
header('X-FRAME-OPTIONS: DENY');
    ...
    if($r=="予約キャンセル") {
        ...
        try {
            ...
            //当日コワーキング
            $sql  = "SELECT * FROM t_member_service_detail LEFT JOIN t_service_set using(`service_start_id`) LEFT JOIN m_service using(`service_id`) JOIN m_member using(`member_id`) where service_flg='1'" ;
            $stmt = $pdo -> prepare($sql);
            $stmt->execute();
