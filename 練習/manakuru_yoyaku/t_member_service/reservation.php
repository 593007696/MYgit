...
<?php if ($pageFlag === 0) : ?>
    ...
    <?php
                ...
                try {
                    ...
                    $stmt->execute(array(':member_id' =>  $_SESSION['member_id']));
                    $result = 0;
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="" method="post">
                                <input type="submit" name="application" class="button" title="reservation" value="受講お申し込み" 　onclick="func1()" />
    ...
    <?php if ($pageFlag === 1) : ?>
        ...
        <?php
                try {
                    ...
                    $sql  = "SELECT t_service_set.service_start_id,
                    t_service_set.store_id,
                    t_service_set_detail.service_nth_detail,
                    t_service_set_detail.service_nth_detail_sub,
                    t_service_set_detail.start_time_detail,
                    t_service_set_detail.end_time_detail,
                    t_service_set_detail.service_day_detail 
                    FROM t_service_set 
                    JOIN t_service_set_detail using(`service_start_id`) 
                    WHERE t_service_set_detail.service_start_id=:service_start_id";
                    $stmt = $pdo->prepare($sql);
